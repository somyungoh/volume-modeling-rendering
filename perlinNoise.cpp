...
const float perlinNoise::eval(const vec3& vec) const
{
...
void perlinNoise::init(void)
{
	...
	for (i = 0; i < B; i++) {
		...
		g1[i] = (float)((randomInt() % (B + B)) - B) / B;

		for (j = 0; j < 2; j++)
			g2[i][j] = (float)((randomInt() % (B + B)) - B) / B;
		normalize2(g2[i]);

		for (j = 0; j < 3; j++)
			g3[i][j] = (float)((randomInt() % (B + B)) - B) / B;
		normalize3(g3[i]);
	}
...
const float perlinNoiseGustavson::eval(const vec3& P) const
{
